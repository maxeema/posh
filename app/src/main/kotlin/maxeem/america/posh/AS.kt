...
class AS : Activity() { //Shortcuts
    ...
    private fun populateMarkets() {
        ...
        D.markets.forEach { m -> createChip(m).apply {
            ...
            if (!m.isDepartment) {
                ...
                if (m.id == "market_all") return@apply
            }
            styleMarket(this)
    ...
    private fun onClick(v: View) = (v.tag as D.Item).let { item->
        println("onClick: ${item.id}, ${item.label}")
        val icon = if (item is D.Market) {
            try {
                val b = resources.openRawResource(U.identifier(item.id, "raw")).use { stream ->
                    BitmapFactory.decodeStream(stream, null, BitmapFactory.Options())!!
                }
                Icon.createWithAdaptiveBitmap(b)
            } catch (e: Exception) {
                e.printStackTrace()
                item.icon
            }
        ...
        if (!S.requestPinned(item.id, item.label, icon))
